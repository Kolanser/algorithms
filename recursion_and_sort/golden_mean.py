"""
https://contest.yandex.ru/contest/24734/problems/M/
Золотая середина
...
Определение: Медиана (https://ru.wikipedia.org/wiki/Медиана_(статистика)) массива чисел a_i —– это такое число, что половина чисел из массива
не больше него, а другая половина не меньше. В общем случае медиану массива можно найти, отсортировав числа и взяв среднее из них.
...
mid_index = len(rez) // 2
if not len(rez) % 2:
    rez = (rez[mid_index] + rez[mid_index - 1]) / 2
    rez = int(rez) if rez.is_integer() else rez
    print(rez)
else:
    print(rez[mid_index])